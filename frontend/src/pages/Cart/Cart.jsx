                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {cartItems.map((item) => (
                            <div key={item._id} className="bg-white p-4 rounded-lg shadow-md">
                                <img src={item.image} alt={item.name} className="w-24 h-24 object-cover rounded-md mx-auto mb-4" />
                                <div className="text-center">
                                    <Link to={`/product/${item._id}`} className='text-pink-500 font-semibold'>{item.name}</Link>
                                </div>
                                <div className="flex justify-between mt-2">
                                    <span className="text-gray-600">Quantity: {item.quantity}</span>
                                    <span className="text-gray-600">Brand: {item.brand}</span>
